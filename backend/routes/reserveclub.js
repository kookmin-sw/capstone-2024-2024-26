...
if (!existingReservationsSnapshot.empty) {
  return res.status(400).json({ error: "The same reservation already exists" });
}
...
// 겹치는 예약이 있는지 확인
if (isOverlapping) {
  return res.status(400).json({ error: "The room is already reserved for this time" });
}
...
reserveClub.get("/reservationclubs/:date", async (req, res) => {
  ...
  try {
    ...
    // 해당 날짜의 모든 예약 내역 반환
    res.status(200).json({ message: "Reservations for the date fetched successfully", reservations });
  } catch (error) {
    // 오류 발생 시 오류 응답
    console.error("Error fetching reservations for the date", error);
    res.status(500).json({ error: "Failed to fetch reservations for the date" });
  }
});
